...
public class Asteroid extends Participant implements ShipDestroyer, BulletDestroyer, AlienDestroyer
{
    ...
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            ...
            // direction.
            int size = getSize() - 1;
            //creates two small asteroids with random speed at the same position
            if (size == 0)
            {
                controller.addParticipant(
                        new Asteroid(RANDOM.nextInt(4), size, getX(), getY(), 3 + RANDOM.nextInt(2), controller));
                controller.addParticipant(
                        new Asteroid(RANDOM.nextInt(4), size, getX(), getY(), 3 + RANDOM.nextInt(2), controller));
            }
            else if (size == 1)
            {
                controller.addParticipant(
                        new Asteroid(RANDOM.nextInt(4), size, getX(), getY(), 4 + RANDOM.nextInt(2), controller));
                controller.addParticipant(
                        new Asteroid(RANDOM.nextInt(4), size, getX(), getY(), 4 + RANDOM.nextInt(2), controller));
            }

            // Inform the controller
            controller.asteroidDestroyed(size + 1, getX(), getY());
        }
    }
}
